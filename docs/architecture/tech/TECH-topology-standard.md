...
`application-http-gateway`: Knowledgebase composes `sdkwork-routes-knowledgebase-*` route crates into one `sdkwork-knowledgebase-standalone-gateway` application public-ingress binary. Shared IAM and appbase SDKs use `platform.api-gateway` unless a standalone profile embeds the required platform adapter.
