...
-- Knowledgebase website aggregate, immutable releases, and host bindings.
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_kb_site_uuid ON kb_site (tenant_id, uuid);
CREATE UNIQUE INDEX IF NOT EXISTS uk_kb_site_space
    ON kb_site (tenant_id, organization_id, space_id) WHERE status = 1;
...
CREATE TABLE IF NOT EXISTS kb_site_release (
    ...
    CHECK (page_count >= 0 AND asset_count >= 0),
    CHECK (status IN (0, 1)),
    CHECK (
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_kb_site_release_content
    ON kb_site_release (tenant_id, site_id, source_content_hash) WHERE status = 1;
CREATE INDEX IF NOT EXISTS idx_kb_site_release_history
    ON kb_site_release (tenant_id, site_id, id DESC) WHERE status = 1;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_kb_site_host_binding_host
    ON kb_site_host_binding (normalized_host) WHERE status = 1;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_kb_site_host_binding_canonical
    ON kb_site_host_binding (tenant_id, site_id) WHERE status = 1 AND canonical = 1;
CREATE INDEX IF NOT EXISTS idx_kb_site_host_binding_site
    ON kb_site_host_binding (tenant_id, site_id, id) WHERE status = 1;
