...
use sdkwork_knowledgebase_product::ports::knowledge_space_store::KnowledgeSpaceStore;
...
async fn sqlite_browser_projection_batches_wiki_page_status_by_logical_path() {
    ...
    apply_sqlite_migration(&pool).await;
    let tenant_id = 9001;
    let wiki_pages = sdkwork_knowledgebase_storage_sqlx::SqliteKnowledgeWikiPageStore::new(
        pool.clone(),
        tenant_id,
    );
    let projections = SqliteKnowledgeBrowserProjectionStore::new(pool, tenant_id);
