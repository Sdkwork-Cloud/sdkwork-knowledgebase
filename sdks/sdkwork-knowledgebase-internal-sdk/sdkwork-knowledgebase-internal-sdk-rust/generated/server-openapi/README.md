# sdkwork-knowledgebase-internal-sdk (Rust)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = client.knowledgebase_internal_wiki().wiki_publications_retrieve(publication_uuid).await?;
    println!("{result:?}");
    Ok(())
}
```
...
- `client.knowledgebase_internal_wiki()` - knowledgebase_internal_wiki API
...
let outcome: Result<(), _> = async {
    let publication_uuid = "1";
    client.knowledgebase_internal_wiki().wiki_publications_retrieve(publication_uuid).await?;
    Ok(())
}.await;
