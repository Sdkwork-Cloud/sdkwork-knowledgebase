...
async fn main() {
    ...
    let router = dev_auth::with_dev_app_auth(runtime.build_full_app_router(), tenant_id, actor_id);
    let listener = tokio::net::TcpListener::bind(&listen_addr)
        .await
        .expect("bind knowledgebase app api listener");
    eprintln!("sdkwork-knowledgebase-app-api listening on {listen_addr}");
    axum::serve(listener, router)
        .await
        .expect("serve knowledgebase app api");
}
