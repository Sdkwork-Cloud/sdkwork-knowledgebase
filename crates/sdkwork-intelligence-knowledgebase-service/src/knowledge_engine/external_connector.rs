...
use sdkwork_knowledgebase_contract::source::{
    dataset_id_from_connector_metadata_json, workspace_slug_from_connector_metadata_json,
    KnowledgeSourceType,
};
...
/// Resolves an AnythingLLM workspace slug for a space from connector sources or env default.
pub async fn resolve_connector_workspace_slug_for_space(
    source_store: &dyn KnowledgeSourceStore,
    space_id: u64,
    expected_implementation_id: &str,
    ...
    for source in sources {
        ...
        if let Some(workspace_slug) =
            workspace_slug_from_connector_metadata_json(source.connector_metadata_json.as_deref())
        {
            return Ok(workspace_slug);
        }
    }

    default_workspace_slug.ok_or_else(|| {
        KnowledgeEngineError::Validation(format!(
            "external search requires kb_source connector metadata workspaceSlug or {config_hint} for space_id={space_id}"
        ))
    })
}
