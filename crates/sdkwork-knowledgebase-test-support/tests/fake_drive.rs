...
async fn fake_drive_puts_and_reads_text_by_object_ref() {
    ...
    let object_ref = drive
        .put_text("okf/index.md", "bundle_index", "# Index")
        .await
        .unwrap();

    assert_eq!(object_ref.logical_path, "okf/index.md");
    assert_eq!(object_ref.object_role, "bundle_index");
...
async fn fake_drive_rejects_missing_objects() {
    let drive = FakeKnowledgeDriveStorage::default();
    let object_ref = drive
        .put_text("okf/log.md", "bundle_log", "# Log")
        .await
        .unwrap();
