...
#[test]
fn chroma_adapter_engine_registers_catalog_ids_when_unconfigured() {
    let engine = ChromaKnowledgeEngine::stub();
    assert_eq!(
        engine.descriptor().implementation_id,
        KnowledgeEngineId::external("chroma").0
    );
    assert_eq!(
        engine.descriptor().agent_provider_id,
        KnowledgeEngineId::external_agent_provider("chroma")
    );
}
