...
-- Production greenfield deployments receive this definition from database/ddl/baseline/postgres.
...
-- `CREATE TABLE IF NOT EXISTS` leaves prior pre-launch compatibility tables untouched. Add the
-- defense-in-depth constraints when they are absent so upgraded and greenfield schemas agree.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'kb_group_knowledge_space_binding'::regclass
          AND conname = 'ck_kb_group_knowledge_space_active_acl_projection'
    ) THEN
        ALTER TABLE kb_group_knowledge_space_binding
            ADD CONSTRAINT ck_kb_group_knowledge_space_active_acl_projection
            CHECK (lifecycle_state <> 'active' OR acl_projection_state = 'active');
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'kb_group_knowledge_space_member'::regclass
          AND conname = 'ck_kb_group_knowledge_space_member_role_access'
    ) THEN
        ALTER TABLE kb_group_knowledge_space_member
            ADD CONSTRAINT ck_kb_group_knowledge_space_member_role_access
            CHECK (
                COALESCE(access_level, '') = CASE member_role
                    WHEN 'owner' THEN 'owner'
                    WHEN 'admin' THEN 'writer'
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'kb_group_knowledge_space_binding'::regclass
          AND conname = 'ck_kb_group_knowledge_space_binding_tenant'
    ) THEN
        ALTER TABLE kb_group_knowledge_space_binding
            ADD CONSTRAINT ck_kb_group_knowledge_space_binding_tenant
            CHECK (tenant_id > 0);
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'kb_group_knowledge_space_binding'::regclass
          AND conname = 'ck_kb_group_knowledge_space_binding_organization'
    ) THEN
        ALTER TABLE kb_group_knowledge_space_binding
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'kb_group_knowledge_space_member'::regclass
          AND conname = 'ck_kb_group_knowledge_space_member_tenant'
    ) THEN
        ALTER TABLE kb_group_knowledge_space_member
            ADD CONSTRAINT ck_kb_group_knowledge_space_member_tenant
            CHECK (tenant_id > 0);
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'kb_group_knowledge_space_member'::regclass
          AND conname = 'ck_kb_group_knowledge_space_member_organization'
    ) THEN
        ALTER TABLE kb_group_knowledge_space_member
    ...
    IF NOT EXISTS (
        ...
        WHERE conrelid = 'kb_group_knowledge_space_event_inbox'::regclass
          AND conname = 'ck_kb_group_knowledge_space_event_inbox_tenant'
    ) THEN
        ALTER TABLE kb_group_knowledge_space_event_inbox
            ADD CONSTRAINT ck_kb_group_knowledge_space_event_inbox_tenant
            CHECK (tenant_id > 0);
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'kb_group_knowledge_space_event_inbox'::regclass
          AND conname = 'ck_kb_group_knowledge_space_event_inbox_organization'
    ) THEN
        ALTER TABLE kb_group_knowledge_space_event_inbox
...
-- The core RLS migration predates this aggregate. New and upgraded PostgreSQL databases must
-- enforce the same tenant session boundary immediately when these tables appear.
DO $$
DECLARE
    table_name text;
BEGIN
